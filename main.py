...
def scan_engine(df, filters: dict):
    temp = df.copy()
...
def integrated_scan(year:int, round_no:str, match_no:int,
    ...
    result["기본조건"] = scan_engine(df, base_filter)
    ...
    result["일반전체"] = scan_engine(df, general_filter)

    # 3. 리그전체
    result["리그전체"] = scan_engine(df, {"리그":row["리그"]})

    # 4. 팀홈
    result["홈팀전체"] = scan_engine(df, {"홈팀":row["홈팀"]})

    # 5. 팀원정
    result["원정팀전체"] = scan_engine(df, {"원정팀":row["원정팀"]})

    # 6. 배당
    result["승배당"] = scan_engine(df, {"승":row["승"]})
    result["무배당"] = scan_engine(df, {"무":row["무"]})
    result["패배당"] = scan_engine(df, {"패":row["패"]})
...
def home():
    ...
    async function load(){
        ...
        data.forEach(m=>{
            ...
            <button onclick="info(${m.년도},'${m.회차}',${m.순번})">정보</button>
            </div>`;
        });
        document.getElementById("list").innerHTML=html;
    }

    async function info(y,r,m){
        let res=await fetch(`/integrated-scan?year=${y}&round_no=${r}&match_no=${m}`,
        {headers:{"Authorization":"Bearer "+token}});
        let d=await res.json();
        alert("기본조건 승: "+d.분포.기본조건.승);
    }
