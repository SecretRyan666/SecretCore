...
def integrated_scan(year:int, round_no:str, match_no:int,
    ...
    base_keys = ["유형","홈원정","일반구분","정역","핸디구분"]

    base = filter_key(df,row,base_keys)
    base_dist = dist(base)

    general_all = filter_key(df,row,["유형","홈원정","일반구분"])
    ...
    odds_win = df[df["승"]==row["승"]]
    odds_draw = df[df["무"]==row["무"]]
    odds_lose = df[df["패"]==row["패"]]
